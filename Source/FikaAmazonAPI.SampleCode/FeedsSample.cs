...
using StandardProductID = FikaAmazonAPI.ConstructFeed.Messages.StandardProductID;
...
public class FeedsSample
{
    ...
    {
        var text = File.ReadAllText(@"C:\Users\tareq\Downloads\Beispiel_Upload.txt");
    ...
    {
        var data = amazonConnection.Feed.GetFeeds(new ParameterGetFeed
        {
    ...
    {
        var data2 = amazonConnection.Feed.GetFeedDocument(
            "amzn1.tortuga.3.92d8fd38-6ccf-49be-979f-6dc27375ea3e.T2DF7HINJ0NRA2");
    }
    ...
    {
        var createDocument = new ConstructFeedService(amazonConnection.GetCurrentSellerID, "1.02");
        ...
        createDocument.AddProductMessage(list);
        var xml = createDocument.GetXML();
    ...
    public async Task SubmitFeedPricingWithSalePrice(string sku, decimal price, decimal salePrice, DateTime startDate,
        DateTime endDate)
    {
    ...
    {
        var createDocument2 = new ConstructFeedService("{SellerID}", "1.02");
    ...
    {
        var ResultFeedDocumentId = string.Empty;
    ...
    private void DisplayProcessingReportMessage(ProcessingReportMessage processingReport)
    {
        ...
        if (processingReport.Result != null && processingReport.Result.Count > 0)
            foreach (var itm in processingReport.Result)
                Console.WriteLine("ResultDescription=" + (itm.AdditionalInfo?.SKU ?? string.Empty) + " > " +
                                  itm.ResultDescription);
    }
}