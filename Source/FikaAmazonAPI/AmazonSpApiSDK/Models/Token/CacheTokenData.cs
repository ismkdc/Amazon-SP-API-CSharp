...
namespace FikaAmazonAPI.AmazonSpApiSDK.Models.Token
{
    public class CacheTokenData
    {
        ...
        {
            if (AWSAuthenticationTokenData != null &&
                AWSAuthenticationTokenData.Expiration.AddSeconds(-60) > DateTime.Now)
        ...
        public TokenResponse GetToken(TokenDataType tokenDataType)
        {
            ...
            else if (tokenDataType == TokenDataType.Grantless) token = GrantlessAccessToken;
        ...
        public static bool IsTokenExpired(int? expiresIn, DateTime? dateCreated)
        {
            if (dateCreated == null)
                return false;
            return DateTime.UtcNow.Subtract((DateTime)dateCreated).TotalSeconds >
                   expiresIn - 60; //Add Margent to a void expaired token
        }
    }
}