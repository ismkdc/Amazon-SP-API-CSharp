...
namespace FikaAmazonAPI.Services
{
    public class CatalogItemService : RequestService
    {
        ...
        {
            return Task.Run(() => ListCatalogItemsAsync(parameterListCatalogItems)).ConfigureAwait(false).GetAwaiter()
                .GetResult();
        }
        ...
        public async Task<IList<Item>> ListCatalogItemsAsync(ParameterListCatalogItems parameterListCatalogItems)
        {
            ...
            if (
                ...
                throw new InvalidDataException(
                    "At least one of Query, SellerSKU, UPC, EAN, ISBN, JAN is also required and cannot be null or empty");

            var list = new List<Item>();

            var parameter = parameterListCatalogItems.getParameters();

            await CreateAuthorizedRequestAsync(CategoryApiUrls.ListCatalogItems, Method.Get, parameter);
            var response =
                await ExecuteRequestAsync<ListCatalogItemsResponse>(RateLimitType.CatalogItems_ListCatalogItems);

            if (response != null && response.Payload != null && response.Payload.Items != null &&
                response.Payload.Items.Count > 0)
        ...
        {
            return Task.Run(() => GetCatalogItemAsyncJson(asin)).ConfigureAwait(false).GetAwaiter().GetResult();
        }
        ...
        public async Task<string> GetCatalogItemAsyncJson(string asin)
        {
            ...
            await CreateAuthorizedRequestAsync(CategoryApiUrls.GetCatalogItem(asin), Method.Get, param);
        ...
        {
            return Task.Run(() => GetCatalogItemAsync(asin)).ConfigureAwait(false).GetAwaiter().GetResult();
        }
        ...
        {
            return Task.Run(() => ListCatalogCategoriesAsync(ASIN, SellerSKU, MarketPlaceID)).ConfigureAwait(false)
                .GetAwaiter().GetResult();
        }

        public async Task<IList<Categories>> ListCatalogCategoriesAsync(string ASIN, string SellerSKU = null,
            string MarketPlaceID = null, CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrEmpty(ASIN))
            ...
            var param = new List<KeyValuePair<string, string>>();
            param.Add(new KeyValuePair<string, string>("MarketplaceId",
                MarketPlaceID ?? AmazonCredential.MarketPlace.ID));
            ...
            await CreateAuthorizedRequestAsync(CategoryApiUrls.ListCatalogCategories, Method.Get, param,
                cancellationToken: cancellationToken);
            var response =
                await ExecuteRequestAsync<ListCatalogCategoriesResponse>(
                    RateLimitType.CatalogItems_ListCatalogCategories, cancellationToken);
        ...
        public AmazonSpApiSDK.Models.CatalogItems.V20220401.Item GetCatalogItem202204(
            ParameterGetCatalogItem parameterGetCatalogItem)
        {
            return Task.Run(() => GetCatalogItem202204Async(parameterGetCatalogItem)).ConfigureAwait(false).GetAwaiter()
                .GetResult();
        }
        ...
        /// </summary>
        public async Task<AmazonSpApiSDK.Models.CatalogItems.V20220401.Item> GetCatalogItem202204Async(
            ParameterGetCatalogItem parameterGetCatalogItem, CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrEmpty(parameterGetCatalogItem.ASIN))
                throw new InvalidDataException("asin is a required property and cannot be null");

            if (parameterGetCatalogItem == null || parameterGetCatalogItem.marketplaceIds == null ||
                parameterGetCatalogItem.marketplaceIds.Count == 0)
            ...
            await CreateAuthorizedRequestAsync(CategoryApiUrls.GetCatalogItem202204(parameterGetCatalogItem.ASIN),
                Method.Get, param, cancellationToken: cancellationToken);
            var response =
                await ExecuteRequestAsync<AmazonSpApiSDK.Models.CatalogItems.V20220401.Item>(
                    RateLimitType.CatalogItems20220401_GetCatalogItem, cancellationToken);

            return response;
        }


        public IList<AmazonSpApiSDK.Models.CatalogItems.V20220401.Item> SearchCatalogItems202204(
            ParameterSearchCatalogItems202204 parameterSearchCatalogItems)
        {
            return Task.Run(() => SearchCatalogItems202204Async(parameterSearchCatalogItems)).ConfigureAwait(false)
                .GetAwaiter().GetResult();
        }

        public async Task<IList<AmazonSpApiSDK.Models.CatalogItems.V20220401.Item>> SearchCatalogItems202204Async(
            ParameterSearchCatalogItems202204 parameter, CancellationToken cancellationToken = default)
        {
            ...
            var list = new List<AmazonSpApiSDK.Models.CatalogItems.V20220401.Item>();

            var param = parameter.getParameters();

            await CreateAuthorizedRequestAsync(CategoryApiUrls.SearchCatalogItems202204, Method.Get, param,
                cancellationToken: cancellationToken);
            var response =
                await ExecuteRequestAsync<ItemSearchResults>(RateLimitType.CatalogItems20220401_SearchCatalogItems,
                    cancellationToken);
            ...
            if (response.Pagination != null && !string.IsNullOrEmpty(response.Pagination.NextToken))
            {
                ...
                while (!string.IsNullOrEmpty(nextToken) &&
                       (!parameter.maxPages.HasValue || totalPages < parameter.maxPages.Value))
                {
        ...
        private async Task<ItemSearchResults> SearchCatalogItemsByNextToken202204Async(
            ParameterSearchCatalogItems202204 parameter, CancellationToken cancellationToken = default)
        {
            var list = new List<AmazonSpApiSDK.Models.CatalogItems.V20220401.Item>();
