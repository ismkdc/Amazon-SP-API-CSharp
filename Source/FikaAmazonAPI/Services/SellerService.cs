...
namespace FikaAmazonAPI.Services
{
    public class SellerService : RequestService
    {
        ...
        {
            return Task.Run(() => GetMarketplaceParticipationsAsync()).ConfigureAwait(false).GetAwaiter().GetResult();
        }

        public async Task<List<MarketplaceParticipation>> GetMarketplaceParticipationsAsync(
            CancellationToken cancellationToken = default)
        {
            await CreateAuthorizedRequestAsync(SellersApiUrls.GetMarketplaceParticipations, Method.Get,
                cancellationToken: cancellationToken);
            var response =
                await ExecuteRequestAsync<GetMarketplaceParticipationsResponse>(
                    RateLimitType.Sellers_GetMarketplaceParticipations, cancellationToken);
